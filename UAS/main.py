...
class BaseMethod():
    ...
    def __init__(self):
        self.raw_weight = {'harga': 8, 'ram': 5, 'kapasitas_baterai': 4, 'processor': 5, 'penyimpanan_internal': 3}
    ...
    @property
    def data(self):
        query = select(LaptopModel.nama_laptop, LaptopModel.harga, LaptopModel.ram, LaptopModel.kapasitas_baterai, LaptopModel.processor, LaptopModel.penyimpanan_internal)
        result = session.execute(query).fetchall()
        print(result)
        return [{'nama_laptop': laptop.nama_laptop, 'harga': laptop.harga, 'ram': laptop.ram, 'kapasitas_baterai': laptop.kapasitas_baterai, 'processor': laptop.processor, 'penyimpanan_internal': laptop.penyimpanan_internal} for laptop in result]
...
class Laptop(Resource):
    ...
    def get(self):
        ...
        data = [{'nama_laptop': laptop.nama_laptop, 'harga': laptop.harga, 'ram': laptop.ram, 'kapasitas_baterai': laptop.kapasitas_baterai, 'processor': laptop.processor, 'penyimpanan_internal': laptop.penyimpanan_internal} for laptop in session.scalars(query)]
